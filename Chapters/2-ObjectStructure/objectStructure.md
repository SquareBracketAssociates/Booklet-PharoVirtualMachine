...
The Pharo VM uses since a couple of years the Spur memory model, designed and implemented by Eliot Miranda for the OpenSmalltalk VM {!citation|ref=Mira18a!}.
...
Since the most common machines nowadays are 64-bit machines, we will focus our presentation on them.
...
Processors usually define also concepts such as _half-word_ and _double-word_.
...
A read/write to an address is said to be aligned when the address is a multiple of the accessed element size, in bytes.
...
However, enforcing the same alignment in all architectures allows an overall code simplification by unifying the 64-bit and 32-bits implementations.
...
Variable-sized tags help us mitigate this problem in 32-bit architectures, in which the loss is of 10% (3 bits out of 32).
...
Figure *@fig:32bitsimm@* illustrates the entire 32-bit tagging schema, with tag bits grayed out:
